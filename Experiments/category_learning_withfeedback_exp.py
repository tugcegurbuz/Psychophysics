...
#index which will be used in giving feedback
feedbback_ind = 0
...
def create_text(textt, relx, rely, fg, font):
	t = Label(text = textt, fg = fg, font = font)
...
def phase2(n, i):
	...
	create_text("Aşağıdaki görsellerden hangisi az önce gördüğünüz görsel ile aynı aileye aittir?", 0.05, 0.01, "black", "Times 18 bold")
...
def get_resp(img):
	...
	create_text("Doğru!", (0.7 - feedbback_ind * 0.2), 0.3, "green", "Times 30 bold")
	screen.update()
	time.sleep(3)
...
def clean_write():
	...
	if count > 4:
		...
		for j in range(len(answers)):
			next_row()
			file.write(row, column, answers[j])
			next_column()
			#file.write(row, column, rts[j])
		close_output_file()
